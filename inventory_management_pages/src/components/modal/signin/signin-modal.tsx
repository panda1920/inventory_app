...
export default function SigninModal({ isOpen, close }: SigninModalProps) {
  ...
  return (
    <CommonModal isOpen={isOpen} close={close} title={title}>
      <Box>
      ...
      <Box>
        {isLogin && (
          <Typography>
            Don&apos;t have an account? <InlineButton onClick={toggleMode}>Signup</InlineButton> to
            the app!
          </Typography>
        )}
        {!isLogin && (
          <Typography>
            Already have an account? <InlineButton onClick={toggleMode}>Login</InlineButton> to the
            app!
          </Typography>
        )}
      </Box>
